...
function parseArgs(argv) {
  const args = { conclusion: "success", title: "Policy-controlled check", summary: "" };
  ...
  for (const required of ["owner", "repo", "sha", "name"]) {
...
async function main() {
  ...
  const check = await api(token, "POST", `repos/${target}/check-runs`, {
    name: args.name,
    head_sha: args.sha,
    status: "completed",
