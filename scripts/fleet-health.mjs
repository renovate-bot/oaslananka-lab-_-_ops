...
function npmTrustedPublisherStatus(packageName) {
  ...
  const result = spawnSync("npm", ["view", packageName, "name", "--json"], { encoding: "utf8" });
  if (result.status !== 0) return "package_not_visible";
...
function markdownFor(report) {
  const lines = [
    ...
    "",
    "| Repo | Dependabot PRs | Dependabot alerts | Drift | Production reviewers | Latest publish/deploy | Trusted publisher |",
    "|---|---:|---|---|---:|---|---|",
  ];
  for (const row of report.repos) {
    lines.push(
      `| \`${row.repository}\` | ${row.dependabot_prs_pending_count} | ${JSON.stringify(row.dependabot_open_count)} | ${row.source_mirror_drift} | ${row.production_environment_required_reviewers_count} | ${row.latest_publish_run_state}${row.latest_publish_run_url ? `<br>${row.latest_publish_run_url}` : ""} | ${row.trusted_publisher_configured} |`,
    );
  }
  return `${lines.join("\n")}\n`;
}
...
export function main() {
  ...
  for (const entry of repos()) {
    ...
    report.repos.push({
      ...
      dependabot_prs_pending_count: (prs || []).length,
