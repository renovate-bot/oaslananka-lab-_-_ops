# Mirror Map

Generated from GitHub repository inventory on 2026-05-09.

Updated on 2026-05-10 after the Group B pilot mirrors were created in `oaslananka-lab`.

Updated on 2026-05-11 to make the source/mirror authority model explicit.

`oaslananka/*` is canonical source-of-truth. `oaslananka-lab/*` is the CI/CD mirror and execution workspace. Validated mirror changes must promote back to `oaslananka/*` unless a repo policy explicitly allows mirror-only closeout.

| Personal canonical source | Lab CI/CD mirror | Sync status |
|---|---|---|
| oaslananka/a2a-mesh | oaslananka-lab/a2a-mesh | pending (main -> main) |
| oaslananka/boardguard | oaslananka-lab/boardguard | pending (main -> main) |
| oaslananka/cifence | oaslananka-lab/cifence | pending (main -> main) |
| oaslananka/codex-app-server-web | oaslananka-lab/codex-app-server-web | pending (main -> main) |
| oaslananka/fovux | oaslananka-lab/fovux | pending (main -> main) |
| oaslananka/kicad-mcp-pro | oaslananka-lab/kicad-mcp-pro | pending (main -> main) |
| oaslananka/kicad-studio | oaslananka-lab/kicad-studio | pending (main -> main) |
| oaslananka/mcp-debug-recorder | oaslananka-lab/mcp-debug-recorder | onboarded (main -> main) |
| oaslananka/mcp-health-monitor | oaslananka-lab/mcp-health-monitor | onboarded (main -> main) |
| oaslananka/mcp-infra-lens | oaslananka-lab/mcp-infra-lens | onboarded (main -> main) |
| oaslananka/mcp-ssh-tool | oaslananka-lab/mcp-ssh-tool | pending (main -> main) |
| oaslananka/oaslananka.github.io | oaslananka-lab/oaslananka.github.io | pending (main -> main) |

## Repos only in personal (not yet mirrored)

| Personal repo | Default branch |
|---|---|
| oaslananka/Airsim101_Yolov10 | main |
| oaslananka/BasicVisualSynthesis | main |
| oaslananka/CV_Cuda_Cpp_Sample | main |
| oaslananka/CamStreamAndroidToPython | main |
| oaslananka/EQPulse | main |
| oaslananka/EQ_Magnitude_Estimation_VIT | main |
| oaslananka/EarthquakePredictionML | main |
| oaslananka/FastUsnSearch | main |
| oaslananka/Filytics | main |
| oaslananka/IoTCloudMonitoring | main |
| oaslananka/LoadTestCyrex | main |
| oaslananka/PPE-Detection-YOLO | master |
| oaslananka/SmartStructureGuardian | main |
| oaslananka/adxl355_spi_library | main |
| oaslananka/dead-simple-queue | main |
| oaslananka/energy-consumption-prediction | main |
| oaslananka/file-header-generator | main |
| oaslananka/helix | main |
| oaslananka/iotfleetops | main |
| oaslananka/mcp-suite | main |
| oaslananka/mhz14a-lib | main |
| oaslananka/multimodel-comparator | main |
| oaslananka/oaslananka | main |
| oaslananka/security-testing-framework | main |
| oaslananka/vscode-toon-tools | main |

## Repos only in org (org-native, no personal counterpart)

| Org repo | Default branch | Note |
|---|---|---|
| oaslananka-lab/CH224A-breakout-for-testing | main | org-native |
| oaslananka-lab/_ops | main | control-plane |
| oaslananka-lab/homebrew-tap | none | org-native |
| oaslananka-lab/scoop-bucket | none | org-native |
| oaslananka-lab/test | main | org test repo; `oaslananka/test` was not resolvable during inventory |
